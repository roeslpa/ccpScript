...
\[
\Pr[\KEMdec(\dk,c)\in\{k,\bot\}\mid (\dk,\ek)\getsr\KEMgen,(k,c)\getsr\KEMenc(\ek))=m]=1
\]
and a correctness definition in the flavor of liveness would be:
\[
\Pr[\KEMdec(\dk,c)\neq\bot\mid (\dk,\ek)\getsr\KEMgen,(k,c)\getsr\KEMenc(\ek))=m]=1\text{,}
\]
...
Here, the adversary can see the values in the dashed boxes, as well as send chosen values (in red) to the victims.
...
Specifying security goal is our second step.
...
For a KEM, one-way~(OW) security means that must be hard to learn the full encapsulated key~$k$.
...
These are strategies or attacks work against every correct construction, simply because correctness enforces that the constructions behave \emph{meaningfully}.
...
The security definitions introduced so far only cover a single (pair) of victims that are attacked by an adversary.

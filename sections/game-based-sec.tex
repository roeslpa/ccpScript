...
\[
\Pr[\KEMdec(\dk,c)\in\{k,\bot\}\mid (\dk,\ek)\getsr\KEMgen,(k,c)\getsr\KEMenc(\ek))]=1
\]
...
To prove that the single-instance IND-CCA game from Figure~\ref{fig:kem:ind} implies its multi-instance generalization from Figure~\ref{fig:kem:ind:mi:corrupt}, one would use a, so called, \emph{hybrid argument}.
...
Doing so, however, yields a factor~$q$ in the final security bound, where $q$ is the total number of instances.
To avoid this degeneration of the security bound, we can use the multi-instance game, for which sometimes tighter security bounds than the above general hybrid proof exist.

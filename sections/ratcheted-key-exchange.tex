...
Thereby, forward security means that the established symmetric key remains secure even if long-term keys of the communication partners were corrupted (later).
...
it is the ability to recover from a previous corruption.
...
For the initial key exchange, one can use the X3DH protocol from Section~\ref{sec:AKE:X3DH}, Signal's Double Ratchet algorithm can be the instantiation of the Ratcheted Key Exchange itself, and AEAD can be used for the encryption of messages.
...
\alert{continue here}
\paragraph{Syntax} An RKE protocol $\RKE=(\RKEinit, \RKEsnd, \RKErcv)$ has the following syntax:
\begin{itemize}
...
Among other dimensions, RKE protocols can be divided into three sub-types, depending on the communication setting they support.
...
This form of interaction is modeled by the syntax notion defined above.

\emph{Sesquidirectional} RKE allows one party full functionality, while the other party can only send ``non-functional'' responses.

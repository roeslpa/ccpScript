...
The key exchange protocols presented in the previous sections offer forward security based on inefficient building blocks or based on interaction.
Thereby, forward security means that the established symmetric key remains secure even if long-term keys of the communication partners were corrupted (\emph{later}).

Ratcheted Key Exchange~(RKE) is a tool that strengthens forward security guarantees and, additionally, offers
post-compromise security~(PCS).
In the time dimension, PCS can be considered the complement of forward security:
it is the ability to recover from a \emph{previous} corruption.
We adapt Figure~\ref{fig:fs:sketch}, which showed the idea of forward security, to show the idea of a protocol that provides both forward \emph{and} post-compromise security (Figure~\ref{fig:rke:pcs_fs_overview}).
...
To achieve post-compromise security, an RKE protocol lets both communication partners regularly generate ephemeral key pairs.
Each resulting public key is shared with the partner, who can encapsulate fresh symmetric keys to the original sender.
Using this encapsulated symmetric key, a new payload message can be encrypted.
By continuously repeating this renewal and replacement of ephemeral key pairs, the effect of a temporary secret key corruption is limited to the period in which this secret key is used:
from the time it is generated until the time it is replaced by the next generated key pair.

In Figure~\ref{fig:rke:overview}, an abstraction of such a two party RKE key refreshing protocol, embedded in a larger communication channel, is provided.
For the initial key exchange, one can, for example, use the X3DH protocol from Section~\ref{sec:AKE:X3DH}, Signal's Double Ratchet algorithm can be the instantiation of the Ratcheted Key Exchange itself, and AEAD can be used for the encryption of payload messages.
...
The first three academic works on RKE contributed a security analyses of the actual Double Ratchet algorithm~\cite{JC:CCDGS20} and a definitional abstraction and simplification of the underlying principles~\cite{C:BSJNS17,C:PoeRos18}.
In this script, we use this simplification for comprehensibility and, thereby, use the following syntax notion.

\newcommand{\RKE}{\mathrm{RKE}}%
\newcommand{\RKEinit}{\RKE.\mathrm{init}}%
\newcommand{\RKEsnd}{\RKE.\mathrm{snd}}%
\newcommand{\RKErcv}{\RKE.\mathrm{rcv}}%


\paragraph{Syntax} An RKE protocol is a tuple algorithms $\RKE=(\RKEinit, \RKEsnd, \RKErcv)$ with local state space~$\stsp$, key space~$\ksp$, and ciphertext space~$\csp$ such that:
\begin{itemize}
    \item $\RKEinit: \emptyset\tor\stsp\times\stsp$\\
    Initializes both communication partners' secret, local states
    \item $\RKEsnd: \stsp\tor\stsp\times\ksp\times\csp$\\
    Sends a ciphertext $c\in\csp$ to the partner to establish a fresh symmetric key~$k\in\ksp$;
    the protocol uses the ciphertext also to update the secret key material stored in local state~$\st\in\stsp$
    \item $\RKErcv: \stsp\times\csp\to\stsp\times\ksp$\\
    Processes ciphertext $c\in\csp$ compute the established symmetric key~$k\in\ksp$;
    the ciphertext may also update secret key material stored in local state~$\st\in\stsp$
\end{itemize}
...
To simplify the notion of RKE, one can gradually limit the communication settings that it supports as follows.
\emph{Bidirectional} RKE covers the standard case in which both session partners equally participate in the communication~(Figure~\ref{fig:RKE:bi}).
This form of interaction is modeled by the syntax defined above.

\emph{Sesquidirectional} RKE lets one party participate without limitations  and full functionality, while the other party can only send ``non-functional'' responses.
...
The third, simplest type of RKE protocols is \emph{unidirectional} RKE, which only allows one party to send.
For simplicity, this is the type of RKE we will focus on in the following.

\subsection{Unidirectional RKE}
An overview of a unidirectional RKE protocol execution is given in Figure~\ref{fig:rke:uni}.
As Bob always only receives ciphertexts from Alice, he can be considered purely deterministic.
Therefore, as he can not share anything based on fresh randomness with Alice, he cannot recover from corruptions.
...
This means that an exposure of the sender state should not leak information about symmetric keys established by the sender before or after that.
Also, an exposure of the receiver state should not leak information about symmetric keys established before that.

Due to receiver's deterministic behavior, receiver state exposures trivially leak all keys established to them thereafter.
There is one exceptions to this:
If sender and receiver were out of sync when the latter was exposed, a receiver exposure must be harmless for keys established by the sender.

Finally, the sender state should only allow for \emph{immediate} impersonations:
If the sender state is exposed immediately before the adversary sends a validly forged ciphertext to the receiver, keys computed by the receiver thereafter are considered insecure.
If, in contrast, such a valid forgery is sent to the receiver with an immediate prior sender exposure, the receiver's subsequent computed keys are required to be secure.
...
A construction for a unidirectional RKE protocol $\URKE$ is given in Figure~\ref{fig:urke:const}.
Using a KEM, this construction continuously encapsulates fresh symmetric keys to the receiver;
these symmetric keys seed the generation of the next KEM key pair.
Using a digital signature, each sent ciphertext as well as the next signature verification key is protected for authenticity.

Since the sender only always has the public KEM encapsulation key in their local state, a corruption thereof does not leak information about past or future encapsulated keys.
The seeded generation of KEM key pairs has two purposes:
It ensures forward security for the receiver, and it makes sure that, when sender and receiver process different ciphertexts (due to an active attack against a sender-to-receiver ciphertext), the key material of both communication participants is diverged and becomes incompatible.
...
The random oracle model is a commonly used idealization for hash or one-way function.
Intuitively, this model says that the output of an (idealized) hash function looks entirely random and is unpredictable until that hash function was evaluated on the input that yields this output;
moreover, this idealization models that knowing the algorithm description of a hash function does not yield an advantage in predicting the output (i.e., only a full evaluation would provide the output for an input).
...
Clearly, real hash functions cannot provide the idealized guarantees of the random oracle model, and there is a list of more or less artificial examples that show the following:
for  construction, proven secure in the random oracle model, uses a real hash function, the security guarantees do not hold

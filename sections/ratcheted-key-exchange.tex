...
The key exchange protocols presented in the previous sections offer forward security based on inefficient building blocks or interaction.
...
By continuously repeating this renewal and replacement of ephemeral key pairs, the effect of a temporary secret key corruption is limited to the period in which this secret key was used:
from the time it was generated until the time it was replaced by the next generated key pair.

In Figure~\ref{fig:rke:overview}, an abstract view on a two party RKE protocol is given.
For the initial key exchange, one can use the X3DH protocol from Section~\ref{sec:AKE:X3DH}, Signal's Double Ratchet algorithm can be the instantiation of the Ratcheted Key Exchange, and AEAD can be used for the encryption of messages.

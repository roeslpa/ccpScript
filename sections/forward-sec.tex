...
Until now, we have used a secret key that was only known by one or more honest parties participating in the various protocols.
...
However, forward security does not guarantee that messages after the compromise are secure.
...
The key can then be used by Alice and Bob to encrypt messages with a symmetric encryption scheme, so it is important that only Alice and Bob can learn the key (and not an entity along the domain path).
...
\paragraph{Correctness} A $\HIBE$ scheme is correct if the secret key $\sk$ delegated to string $\mathit{id}=\mathit{id}_1||\dots||\mathit{id}_l$ can decapsulate the ciphertext $c$ in $(k,c) \getsr \HIBEenc(\pk,\mathit{id})$.

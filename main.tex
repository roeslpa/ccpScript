...
This document is based on the manuscript from the lecture sessions, which was lifted into \LaTeX\ with strong contributions by Jannes Rätsch.
It is not (yet) a full course script.
...
\begin{tabularx}{\linewidth}{l X}\toprule
    ...
    $[n]$ & The set $\{1,\dots,n\}$\\
    $\PS(X)$ & Power set of set~$X$ (i.e., each element of $\PS(X)$ is set of elements from set~$X$)\\
    $(X)^+$ & Set of non-trivial vectors of elements from set~$X$\\
    ...
    $X\coloneqq Y$ & Defines function/algorithm $X$ as $Y$\\
    $X[\cdot]\gets x$ & Assigns all entries of array~$X$ with default value~$x$\\
    Invoke $X$ & Executes algorithm $X$\\
    Return $x$ & Terminates a running algorithm or oracle with output~$x$ that is given to the calling instance or adversary, resp.\\
    Stop with $x$ & Terminates the running experiment with final output~$x$\\
    Require $x$ & Ends the algorithm, resp.~oracle, if expression~$x=\F$\\
    $\Pr[X]$ & Probability of event $X$\\
    $\advA$ & Adversary aka.\ Attacker\\
    $\advR$ & Reduction\\
    $\Adv_\mathrm{C}^\mathrm{gm}(\advA)$ & Advantage, which is probability of adversary~$\advA$ in breaking construction~$\mathrm{C}$ in game~$\mathrm{gm}$ non-trivially\\
    \bottomrule
\end{tabularx}

\section{Cheat Sheet}
Start early in this course with learning and mapping terms and acronyms to their meaning.
Use a table similar to the following, bring it to every session and extend it with new concepts that you want to remember.

\noindent
% l@{\quad}: add a "quad" space after the first left-aligned (l) column --> Better spacing between "IND-CPA" and the second column.
\begin{tabularx}{\linewidth}{l@{\quad}X}\toprule
    \textbf{Term} & \textbf{Meaning}\\\midrule
    Syntax & Abstract input and output interfaces of the algorithms that belong to a primitive.\\
    IND-CPA & \underline{Ind}istinguishability (of encrypted messages/keys/\dots) under \underline{c}hosen \underline{p}laintext \underline{a}ttacks;
    a ciphertext should not reveal a single bit about the encrypted plaintext and, when aiming to learn a plaintext bit, the attacker can observe ciphertexts for maliciously chosen plaintexts.\\
...
\[
\Adv_\DHgr^\dlog(\advA)\coloneqq\Pr[\advA(\DHg,\DHp,\DHg^x)=x\mid x\getsr\ZZ_\DHp^*]\text{.}
\]

...
\begin{tabular}{lllll}
    \algbox{4cm}{%
        ...
        $(\dk,\ek)\getsr\KEMgen$\skiplines{2}
        %
        $(k^*, c^*)\getsr\KEMenc(\ek)$\skiplines{2}
        %
        If $c\neq c^*:$\\
        ...
        Stop with~$0$}&
    \arrbox{1cm}{%
        $\xrightarrow{\ek}$\skiplines{2}
        $\xrightarrow{c^*}$\skiplines{2}
        $\xleftarrow{c}$\\
        $\xrightarrow{k}$\\
        $\xleftarrow{k'}$\\
    ...
    \arrbox{1cm}{
        ...
        $\xleftarrow{d}$}&
    \algbox{2.5cm}{%
        \textbf{Adversary} $\advD_4^5$\skiplines{8}\strut}
\end{tabular}